...
df = df.dropna(subset=["X (mm)", "Y (mm)"])

# Convert the columns to numeric arrays.
x_data = pd.to_numeric(df["X (mm)"], errors="raise").to_numpy()
y_data = pd.to_numeric(df["Y (mm)"], errors="raise").to_numpy()
